...
function [e, J, gamma] = finalerrorAndJacobian(x, z)
    ...
    for i = 1:4
        ...
        J(:, i) = (t2v((v2t(meas)^-1 * T_pred_delta_plus)) - t2v((v2t(meas)^-1 * T_pred_delta_minus))) / (2e-5);
    endfor
    ...
    for i = 1:3
    	...
    	J(:, 4+i) = (t2v(v2t(meas)^-1 * T_delta_s_plus^-1 * v2t(predictTricycle(traction_ticks, steer_ticks, delta_tau, x_kin)) * T_delta_s_plus) - t2v(v2t(meas)^-1 * T_delta_s_minus^-1 * v2t(predictTricycle(traction_ticks, steer_ticks, delta_tau, x_kin)) * T_delta_s_minus)) / (2e-5); 	
    endfor 
endfunction
